...
DecoderOut = namedtuple(
    "ReorderCTCDecoderOut",
    ["output_tokens", "output_scores", "attn", "step", "max_step", "history"],
)

def initialize_output_tokens(decoder_input):
    ...
    initial_output_scores = decoder_input.new_zeros(
        *decoder_input.size()
    ).float()
...
class ReorderCTCGenerator(object):
    def __init__(
        self,
        tgt_dict,
        models=None,
    ...
    def generate(self, models, sample, prefix_tokens=None, constraints=None):
        ...
        encoder_out, for_ctc, source_mask, encoder_tgt_mask, blank_idx=model.forward_encoder([src_tokens, src_lengths])
        decoder_input = self.beta_inverse(F.log_softmax(for_ctc, dim=-1).argmax(-1), blank_idx).cuda()

        encoder_output_tokens = decoder_input.clone()
        encoder_output_scores = torch.zeros(encoder_output_tokens.size()).cuda()


        prev_decoder_out = initialize_output_tokens(decoder_input)
        ...
        prev_output_tokens = decoder_input.clone()
        ...
        for step in range(self.max_iter + 1):
            ...
            # 输出encoder的输出
            finalized_idxs = sent_idxs[terminated]
            finalized_tokens = encoder_output_tokens[terminated]
            finalized_scores = encoder_output_scores[terminated]
            finalized_attn = (
                None
                if (decoder_out.attn is None or decoder_out.attn.size(0) == 0)
                else decoder_out.attn[terminated]
