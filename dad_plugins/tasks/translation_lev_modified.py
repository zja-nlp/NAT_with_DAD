...
class TranslationLevenshteinConfig(TranslationConfig):
    ...
    choose_data: str = field(
        default="iwslt", metadata={"help": "what dataset to use"}
    )
...
class TranslationLevenshteinModifiedTask(TranslationTask):
    ...
    def train_step(
        ...
        if hasattr(self.cfg, 'choose_data') and self.cfg.choose_data == "iwslt":
            sample["glat"] = {"context_p": 0.5}
        else:
