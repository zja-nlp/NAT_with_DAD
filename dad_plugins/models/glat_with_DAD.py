...
class Glat(FairseqNATModel):
    ...
    def add_args(parser):
        ...
        parser.add_argument(
            "--use-hidden-state-input",
            action="store_true",
            help="whether to use hidden state as input",
        )
        parser.add_argument(
            "--input-transform",
            action="store_true",
    ...
    def forward(
        ...
        # encoding
        if hasattr(self.args, 'curriculum_type') and self.args.curriculum_type == 'at_forward':
            prev_output_tokens = at_prev_output_tokens
            # AT阶段不做 glancing sample 也就是第一轮解码
            # for first decoding to replace
            # rep_tgt_tokens = at_prev_output_tokens
            ...
            if glat and tgt_tokens is not None:
                with torch.no_grad():
                    ...
                    glat_info = {
                        ...
                        "glat_accu": torch.true_divide(same_num.sum(), seq_lens.sum()).item(),
